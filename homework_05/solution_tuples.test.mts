...
import {TuplesImgProcessor, TraverseMode} from './solution_tuples.mts';

// size
{
    const img = new TuplesImgProcessor(3, 4);
    assert.deepEqual(img.size, [3, 4]);
}

// getPixel возвращает нулевой пиксель по умолчанию
{
    const img = new TuplesImgProcessor(2, 2);
...
{
    const img = new TuplesImgProcessor(3, 3);
...
console.log('solution_tuples: все тесты прошли');
