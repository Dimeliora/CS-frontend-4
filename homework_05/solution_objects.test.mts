...
import {ObjectsImgProcessor, TraverseMode} from './solution_objects.mts';

// size
{
    const img = new ObjectsImgProcessor(3, 4);
    assert.deepEqual(img.size, [3, 4]);
}

// getPixel возвращает нулевой пиксель по умолчанию
{
    const img = new ObjectsImgProcessor(2, 2);
...
{
    const img = new ObjectsImgProcessor(3, 3);
...
console.log('solution_objects: все тесты прошли');
