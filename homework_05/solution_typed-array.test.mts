...
import {TypedArrayImgProcessor, TraverseMode} from './solution_typed-array.mts';

// size
{
    const img = new TypedArrayImgProcessor(3, 4);
    assert.deepEqual(img.size, [3, 4]);
}

// getPixel возвращает нулевой пиксель по умолчанию
{
    const img = new TypedArrayImgProcessor(2, 2);
...
{
    const img = new TypedArrayImgProcessor(3, 3);
...
// Uint8Array обрезает значения до диапазона 0–255
{
    const img = new TypedArrayImgProcessor(2, 2);
    img.setPixel(0, 0, [300, 0, 0, 0]);
    assert.deepEqual(img.getPixel(0, 0), [44, 0, 0, 0]); // 300 & 0xFF = 44
}
...
console.log('solution_typed-array: все тесты прошли');
