...
export class FlatArrayImgProcessor implements PixelStream {
    ...
    constructor(width: number, height: number) {
        ...
        this.buffer = new Array<number>(width * height * FlatArrayImgProcessor.PIXEL_DATA_LENGTH).fill(0);
    }
    ...
    private getPixelIndex(x: number, y: number): number {
        ...
        return (x + y * width) * FlatArrayImgProcessor.PIXEL_DATA_LENGTH;
    }
    ...
    public forEach(mode: TraverseMode, callback: (rgba: RGBA, x: number, y: number) => void): void {
        ...
        if (mode === TraverseMode.RowMajor) {
            while (y < height) {
                ...
                idx += FlatArrayImgProcessor.PIXEL_DATA_LENGTH;
                x += 1;
        ...
        while (x < width) {
            ...
            idx += width * FlatArrayImgProcessor.PIXEL_DATA_LENGTH;
            y += 1;

            if (y >= height) {
                y = 0;
                x += 1;
                idx = x * FlatArrayImgProcessor.PIXEL_DATA_LENGTH;
            }
        }        
    }
}
